...
function pull(repo, ops, cb) {
  ...
  // refactor this
  ops.status = ops.status || (function(){ })
  ops.progress = ops.progress || (function(){ })

  async.waterfall([
    
    function(fn){
      ops.status('Obteniendo información ...')
      ops.progress(0)
      repo.fetch(ops, fn)
    },
    function(fn){
...
function __down(repo, jobs, ops, cb){
  ...
  __readRemote(repo, ops, function(err, remote){
    ...
    var tasks = objs.map(function(inf){
      return function(fn) {
        __downfile(ops, repo, inf, true, fn)
      }
    })
...
function __downfile(ops, repo, obj, progress, cb){
  ...
  ops.status('Obteniendo ' + repo.engine.cutUid(obj.uid))
  ops.progress(0)
  ...
  http.get(obj.from, function (res){
    bar = statusBar.create ({ total: res.headers["content-length"] })
          .on("render", function (stats){
            ...
            ops.status('Obteniendo ' + repo.engine.cutUid(obj.uid) + ' ' + this.format.percentage (stats.percentage).trim ())
            var pp = stats.percentage * 100
