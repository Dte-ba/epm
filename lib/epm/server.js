...
module.exports = function(Epm) {
  ...
  EpmServer.prototype.listen = function(ops){
    ...
    self.findRepositories(function(err, direpms){
      ...
      var epms = direpms.map(function(edir){
        var e = new Epm(edir);
        e.use('engines', self.engines);
        // a new repo was instanced
        self.emit('repo.instanced', {name: e.REPONAME, repo: e});
        return e;
  ...
  /**
   * 
   * Register a new Ready repository
   *
   * @param {Object} ops
   */
  EpmServer.prototype.register = function(repo){
    var self = this;

    if (self.repos[repo.REPONAME] != null) return;
    
    self.repos[repo.REPONAME] = repo;

    self.routes["/" + repo.REPONAME + ".epm"] = function(req, res){
