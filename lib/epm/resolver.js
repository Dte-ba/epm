...
EpmResolver.prototype.request = function(req) {
  ...
  if (Object.keys(query).length === 1 && query.expand !== undefined)
    return self.metadataExpand(query.expand);
  ...
  if (query.content !== undefined){
    return self.contentRequested(query.content, req);
  }
 ...
 * @param {Object} req
 */
EpmResolver.prototype.metadataExpand = function(expand) {
  var self = this;
  ...
  self.repo.packages.execQuery('all', function(err, data){
    if (err){
      self.emit('error', err);
      return;
    }
    if (typeof expand.split !== 'function' ) {
     self.emit('error', new Error('`expand` must be an string'));
      return; 
    }
    ...
    self.emit('complete', {type: 'json', data: res});
...
EpmResolver.prototype.fileRequested = function(uid) {
  ...
  self.emit('complete', {uid: uid, type: 'file', filename: filename, stats: stats, attached: uid + path.extname(filename) });
...
EpmResolver.prototype.assetRequested = function(uid, asset) {
  ...
  self.getMetadata(uid, info, eng, function(err, meta){
    if (err) {
      self.emit('error', err)
      return;
    };

    eng.asset(self.repo, info, meta, asset, function(err, filename){
      if (err) {
        self.emit('error', err)
        return;
      };
      ...
      self.emit('complete', {uid: uid, type: 'file', filename: filename, stats: stats, attached: path.basename(filename) });
...
EpmResolver.prototype.contentRequested = function(uid) {
  var self = this;
  ...
  self.getMetadata(uid, info, eng, function(err, meta){
    ...
    eng.content(self.repo, info, meta, function(err, data){
      if (err) {
        self.emit('error', err)
        return;
      };
