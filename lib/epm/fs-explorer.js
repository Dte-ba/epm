...
Explorer.prototype.watch = function() {
  ...
  self.fs.saveOnSet = true;

  if (self.isWatching === true) { return self; }

  watch.createMonitor(self.PATH, {filter: _filter}, function (monitor) {
  ...
  function _filter(f){
    if (path.dirname(f) === self.PATH) return true;
...
Explorer.prototype._handler = function(type, info) {
  ...
  fileStatus(prev, info.stat, filename, self.PATH, function(err, data) {
    ...
    switch(data.code){
       case -1: // file deleted
        ev += "deleted";
        res[filename] = _.clone(prev);
       break;
       case  0: // file unchange
        ev += "unchange";
        res[filename] = prev;
       break;
       case  1: // file added
...
/**
 * 
 * Gey type from status code
 *
 * @param {int} code
 */
Explorer.prototype.getType = function(code) {

  switch(code){
     case -1: return "file.deleted";
     case  0: return "file.unchange";
     case  1: return "file.added";
     case  2: return "file.change";
  }
