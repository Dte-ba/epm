...
var Epm = module.exports = function(dir, ops) {
  ...
  self.tags = new EpmTags(self.fs);
  self.packages = new EpmPackages(self);
...
// expose WordUtils
Epm.wordsUtils = require('./utils/words.js');
 ...
 *
 */
Epm.prototype.use = function(pluginname, plugin) {
  var self = this;

  if (pluginname === 'engines' && plugin instanceof Array) {
    plugin.forEach(function(e){
      self.plugins.register(e.name, e.engine);
    });
    return self;
  }

  self.plugins.register(pluginname, plugin);

  return self;
}

Epm.prototype.createResolver = function() {
  var self = this;

  return new EpmResolver(self);
}